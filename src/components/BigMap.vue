<template>
  ...
  >
    <template v-for="(ciudades, index) in geoLocArray" :key="ciudades.id">
      ...
      <MapboxMarker
        v-if="ciudades.id !== 0"
        :lng-lat="[ciudades.lon, ciudades.lat]"
        popup
      >
        <template v-slot:popup>
          <p class="title is-6">{{ ciudades.blogName }}</p>
          <p class="subtitle is-7">{{ niceDate(ciudades.blogDate) }}</p>
          ...
          <RouterLink
            :to="'/post/' + ciudades.id"
            :data-id="ciudades.id"
            @click="$emit('popupClicked', ciudades.id)"
            >link to blog</RouterLink
          >
        </template>
      </MapboxMarker>
    </template>
  ...
  watch: {
    cities() {
      for (let i = 0; i < this.cities.length; i++) {
        fetch(
          `https://api.openweathermap.org/geo/1.0/direct?q=${this.cities[i].city}&limit=1&appid=${this.OPEN_WEATHER_API_KEY}`
        )
          .then((res) => res.json())
          .then((locations) => {
            this.geoLocArray.push({
              id: this.cities[i].id,
              blogName: this.cities[i].title,
              blogDate: this.cities[i].visitingdate,
              authorPic: this.cities[i].authorsimage,
              authorName: this.cities[i].author,
              lon: locations[0].lon,
              lat: locations[0].lat,
            });
          });
